...
const ConflictExplanationPanel = () => {
  const conflictPaths: ConflictPath[] = [
    {
      id: "1",
      path: ["Sequoia Capital", "John Smith", "TechCorp Inc.", "Acme Technologies"],
      severity: "high",
      explanation: "Sequoia Capital has invested in John Smith, who serves as CEO of TechCorp Inc. TechCorp has an ongoing advisory relationship with Acme Technologies, the target company under review. This creates a potential conflict as investment decisions may be influenced by existing business relationships.",
    },
    {
      id: "2",
      path: ["Sequoia Capital", "TechCorp Inc.", "Sarah Williams", "Acme Technologies"],
      severity: "medium",
      explanation: "Sequoia holds a board seat at TechCorp Inc., where Sarah Williams previously worked as an advisor. Sarah is now a co-founder at Acme Technologies. This indirect connection may influence objectivity in investment decisions.",
    },
    {
      id: "3",
      path: ["Sequoia Capital", "TechCorp Inc."],
      severity: "low",
      explanation: "Direct investment relationship exists between Sequoia Capital and TechCorp Inc. While not directly connected to the target, this relationship should be disclosed for transparency.",
    },
  ];
  ...
  const [selectedPath, setSelectedPath] = useState<string>("1");
                  ...
                  {selectedConflict.path.map((node, i) => (
                    <span key={i} className="flex items-center">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        i === 0 
                          ? "bg-node-investor/10 text-node-investor" 
                          : i === selectedConflict.path.length - 1
                          ? "bg-node-target/10 text-node-target"
